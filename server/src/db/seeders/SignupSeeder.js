...
class SignupSeeder {
  static async seed() {
    ...
    const plan5 = await Plan.query().findOne({ name: "Dare to play Betrayal at Baldur's Gate!"})
    const plan6 = await Plan.query().findOne({ name: "Glory awaits in Risk!"})
    const plan7 = await Plan.query().findOne({ name: "Get ready for Carcassonne!" })
    ...
    await Signup.query().insert({ userId: user2.id, planId: plan6.id})
    ...
    await Signup.query().insert({ userId: user3.id, planId: plan6.id})
