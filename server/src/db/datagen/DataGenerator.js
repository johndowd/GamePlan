...
class DataGenerator {
  ...
  static async generateCommentAndJoin() {
    const user = await User.getRandomAiUser()
    const plan = await Plan.getRandomPlan()
    ...
    const comment = await CommentGenerator.generate(user, plan, game)
